...
class IQ_Option:
    ...
    def __get_binary_open(self):
        # for turbo and binary pairs
        binary_data = self.get_all_init_v2()
        binary_list = ["blitz","binary", "turbo"]
        if binary_data:
            for option in binary_list:
                if option in binary_data:
                    for actives_id in binary_data[option]["actives"]:
        ...
        for dirr in ["blitz","binary", "turbo"]:
    ...
    def get_binary_option_detail(self):
        ...
        init_info = self.get_all_init()
        for actives in init_info["result"]["blitz"]["actives"]:
            name = init_info["result"]["blitz"]["actives"][actives]["name"]
            name = name[name.index(".") + 1:len(name)]
            detail[name]["blitz"] = init_info["result"]["blitz"]["actives"][actives]
    ...
    def position_change_all(self, Main_Name, user_balance_id):
        instrument_type = ["cfd", "forex", "crypto",
                           "digital-option", "turbo-option", "binary-option","blitz-option"]
...
##############################################################################################
    ...
    def buy_by_raw_expirations(self, price, active, direction):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        # ใช้ request_id แบบสุ่มเพื่อป้องกันการซ้ำ
        req_id = str(randint(0, 10000))
        
        # ถ้า expired ไม่ได้ระบุ ให้คำนวณจาก server timestamp + 30 วินาที (default สำหรับ Blitz)
        # if expired is None:
        #     expired = int(self.api.timesync.server_timestamp) + 30
        ...
        # เรียก API สำหรับ Blitz
        # self.api.blizt(
        #     float(price),
        #     OP_code.ACTIVES[active],
        #     direction.lower(),
        #     option,  # ค่า option สำหรับ Blitz (เช่น 5)
        #     int(expired),
        #     request_id=req_id
        # )
        self.api.blizt(price=price, act=active, direction=direction,  request_id=req_id)

        start_t = time.time()
        id = None
        self.api.result = None

        # ลด timeout เป็น 2 วินาทีเพื่อความเร็ว
        while self.api.result is None or id is None:
            try:
                if "message" in self.api.buy_multi_option[req_id].keys():
                    logging.error(f"**warning** buy failed: {self.api.buy_multi_option[req_id]['message']}")
                    return False, self.api.buy_multi_option[req_id]["message"]
            except:
                pass
            try:
            ...
            if time.time() - start_t >= 2:
                logging.error('**warning** buy late 2 sec')
                return False, None

        return self.api.result, self.api.buy_multi_option[req_id]["id"]
    # def bbuy(self, price, act, direction, exp_value):
    #     """
    #     High-level function to buy a Blitz option.

    #     :param price: Investment amount
    #     :param act: Asset ID (like 2301)
    #     :param direction: 'call' or 'put'
    #     :param exp_value: Expiry value (e.g., 335795)
    #     :return: (success: bool, option_id: str or None)
    #     """
    #     self.api.buy_blitz_result = None
    #     self.api.buy_blitz_id = None

    #     request_id = str(randint(10000, 99999))

    #     # Initialize result tracking
    #     if not hasattr(self.api, 'buy_blitz_tracker'):
    #         self.api.buy_blitz_tracker = {}
    #     self.api.buy_blitz_tracker[request_id] = {"id": None, "result": None}

    #     # Send the buy request
    #     self.api.blizt(price=price, act=act, direction=direction, exp_value=exp_value, request_id=request_id)

    #     start_time = time.time()
    #     timeout = 10  # seconds

    #     while time.time() - start_time < timeout:
    #         tracker = self.api.buy_blitz_tracker.get(request_id)
    #         if not tracker:
    #             continue

    #         if tracker.get("id") is not None and tracker.get("result") is not None:
    #             success = tracker["result"] == "success"
    #             return success, tracker["id"]

    #         time.sleep(0.1)

    #     logging.error('**warning** buy_blitz timed out after 10 seconds')
    #     return False, None
    def bbuy(self, price, ACTIVES, ACTION, expirations):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        # req_id = "buy"
        req_id = str(randint(0, 10000))
    ...
    def buy(self, price, ACTIVES, ACTION, expirations):
        ...
        return self.api.result, self.api.buy_multi_option[req_id]["id"]
    def buy_blitz0(self, ACTIVES, direction, price):
        """
        Blitz Option ซื้อแบบใหม่
        :param asset_id: เช่น 76 สำหรับ Ondo OTC
        :param direction: "call" หรือ "put"
        :param price: จำนวนเงินลงทุน
        """
        req_id = "blitz_" + str(randint(1000, 9999))
        self.api.buy_blitz(OP_code.ACTIVES[ACTIVES], str(direction), float(price), req_id)

    def buyBlitz(self, price, ACTIVES, ACTION, duration=30):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        req_id = str(randint(0, 10000))
        ...
        # ลด timeout สำหรับ Blitz ให้เร็วขึ้น (เช่น 2 วินาที)
        while self.api.result is None or id is None:
            try:
                if "message" in self.api.buy_multi_option[req_id].keys():
                    return False, self.api.buy_multi_option[req_id]["message"]
            ...
            try:
                ...
                pass
            if time.time() - start_t >= 2:  # ลด timeout จาก 5 เป็น 2 วินาที
                logging.error('**warning** buy late 2 sec')
                return False, None
